...
{
    private int startX; private int startY; private int width; private int height;
    ...
    public Pair<Coordinate, Direction> randomOfWall()
    {
        ...
        if (c.getX() == startX - 1) d = Direction.LEFT;
        if (c.getX() == startX + width + 1) d = Direction.RIGHT;
        if (c.getY() == startY - 1) d = Direction.DOWN;
        if (c.getY() == startY + height + 1) d = Direction.UP;
        if (d == null)
        {
    ...
    {
        return new Room(this.startX - 1, this.startY - 1, this.width + 2, this.height + 2).stream().filter(c -> c.getX() == startX - 1 || c.getY() == startY - 1 || c.getX() == startX + width + 1 || c.getY() == startY + height + 1).collect(Collectors.toCollection(ArrayList::new));
    }

    public ArrayList<Coordinate> wallsExcludingCorners()
    {
        return this.walls().stream().filter(c -> !(
                (c.getX() == startX - 1 || c.getX() == startX + width + 1) && (c.getY() == startY - 1 || c.getY() == startY + height + 1)
